%Battery charging with qubits
%first test script

%% Parameters

N = 200; %highest battery level
theta = pi/4; %swap angle (pi/2 = full swap)

q = 0.25; %charge qubit ground-state occupation
...
Edistcoh(1,:) = diag(reshape(rcoh,N+1,N+1)); %distribution 
for k=1:tmax
    ...
    Ecl(k+1) = sum(n.*pcl);
    Edistcoh(k+1,:) = diag(reshape(rcoh,N+1,N+1)); %distribution 
    Edistinc(k+1,:) = diag(reshape(rinc,N+1,N+1)); %distribution 
...
%% Check energy distribution after some time
...
Pcoh = real(rcoh(1:N+2:end));
Pinc = real(rinc(1:N+2:end));

toc

%compare to test analytics (stationary phase)
F = @(m,s)( sqrt(abs(m))./(2*s).*( besseli(-1/4,(m./s/2).^2,1) + sign(m).*besseli(1/4,(m./s/2).^2,1) ) );
Ptest = 1/sqrt(4*pi*c*W*tmax) * ( F(n-n0-tmax*(v-c*W),sqrt(tmax*(p-(v-c*W)^2))) + F(-n+n0+tmax*(v+c*W),sqrt(tmax*(p-(v+c*W)^2))) );

%plot to greater interval to check long tails
nt = (-200:1:600).';
Pt =  1/sqrt(4*pi*c*W*tmax) * ( F(nt-n0-tmax*(v-c*W),sqrt(tmax*(p-(v-c*W)^2))) + F( -(nt-n0-tmax*(v+c*W)),sqrt(tmax*(p-(v+c*W)^2))) );


%manual integral over Gaussians (should be better than stationary phase approx.)
G = @(phi)( 1./sqrt(2*pi*tmax*(p-(v-c*W*sin(phi)).^2)) .* exp( -1/2/k*(n-n0-k*(v-c*W*sin(phi))).^2./(p-(v-c*W*sin(phi)).^2) ) );

tic
Ptest2 = 1/2/pi * integral(G,-pi,pi,'ArrayValued',true);
toc
