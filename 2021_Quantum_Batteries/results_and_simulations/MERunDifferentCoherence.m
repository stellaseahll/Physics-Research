function [energy, rhof] = MERunDifferentCoherence(q,theta,c,N,tmax,rho0)
%modified from ladderCharging to take input and output
%c: degree of coherence (c=1 superposition, c=0 mixture)
%% Parameters
...
E(1) = sum(n.*r(1:N+2:end));
% Einc(1) = sum(n.*rinc(1:N+2:end));
% Ecl(1) = sum(n.*pcl);
Erg(1) = E(1) - passE(reshape(r,N+1,N+1));
% Erginc(1) = Einc(1) - passE(reshape(rinc,N+1,N+1));
Ergdeph(1) = E(1) - passE( diag(diag(reshape(r,N+1,N+1))) );
...
%     distinc(k+1,:) = diag(reshape(rinc,N+1,N+1)); %distribution 
    Erg(k+1) = E(k+1) - passE(reshape(r,N+1,N+1));
%     Erginc(k+1) = Einc(k+1) - passE(reshape(rinc,N+1,N+1));
    Ergdeph(k+1) = E(k+1) - passE( diag(diag(reshape(r,N+1,N+1))) );
%     trcoh(k+1) = sum(rcoh(1:N+2:end));
%     trinc(k+1) = sum(rinc(1:N+2:end));
    
end

energy = [E; Erg; Ergdeph];
% incE = [Einc; Erginc];
rhof = reshape(r,N+1,N+1);
