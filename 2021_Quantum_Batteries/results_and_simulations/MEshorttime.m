function [t cohE] = MEshorttime(q,N,tmax)
...
%coherent trafo
Lcoh = - 1i*sqrt(q*(1-q)) * ( spLeftMultiply(Hd) - spRightMultiply(Hd));
...
Ergcoh=zeros(1,1001);
Erginc=zeros(1,1001);
Ergdeph=zeros(1,1001);
Ecl=zeros(1,1001); %for sanity check
trcoh = ones(1,1001);
trinc = ones(1,1001);
...
dt = tmax/1000;
% expL = expm(Lcoh*dt);
[X t] = expmv_tspan(Lcoh,rcoh,0,tmax,1000);
