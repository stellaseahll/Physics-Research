...
classdef model3SpinsFridge < handle
    ...
    methods
        ...
        function obj = model3SpinsFridge(G,Ts,ks,Es,Btype,Itype,Mtype,J)
            if nargin>=1
                obj.g = G;
            end
            if nargin>=2
...
%             n3 = nbar(obj,obj.E3,obj.T3); 
            ...
            switch obj.meType
                case 'local'
                    ...
                    n2 = nbar(obj,obj.E2,obj.T2);
                    n3 = nbar(obj,obj.E3,obj.T3);
                    obj.LOps{1} = sqrt(obj.k1*(n1+1)) * kron( s_m, kron(Id, Id) );
                    obj.LOps{2} = sqrt(obj.k1*n1) * kron( s_p, kron(Id, Id) );
                    obj.LOps{3} = sqrt(obj.k2*(n2+1)) * kron( Id, kron(s_m, Id) );
                    obj.LOps{4} = sqrt(obj.k2*n2) * kron( Id, kron(s_p, Id) );
                    obj.LOps{5} = sqrt(obj.k3*(n3+1)) * kron( Id, kron(Id, s_m) );
                    obj.LOps{6} = sqrt(obj.k3*n3) * kron( Id, kron(Id, s_p) );
                otherwise
                    obj.bathIdx = 0;
                    switch obj.bathType
                        case 'cubic'
...
%         end
        ...
        function y = timeEvolve(obj,rhoIn,dt,tmax)
            ...
            rhoVec = reshape(rhoIn,[64,1]);
                 permMat = getPermMat(obj);
                [obj.rhoH, obj.rhoCW] = ptrace(obj.rho,obj.dim,obj.dim^2);
                [obj.rhoHC, obj.rhoW] = ptrace(obj.rho,obj.dim^2,obj.dim);
                [obj.rhoHW, obj.rhoC] = ptrace(permMat*obj.rho*permMat,obj.dim^2,obj.dim);
...
%             rSS = obj.rho;
%             LL = obj.LOps{j};
%             Q = trace( obj.H * ( LL*(rSS*(LL')) - 0.5*(LL'*LL)*rSS - 0.5*rSS*(LL'*LL) ) );
%             
%         end
%         
%         function Q = getTotalHeatFlow(obj,bath) %Computes heat flow from each bath 
%             a = obj.bathIdx(bath)+1;
%             b = obj.bathIdx(bath+1);
%             Q = 0;
%             for k = a:b
