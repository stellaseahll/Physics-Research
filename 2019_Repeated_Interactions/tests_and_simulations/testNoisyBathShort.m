%run script to get time evolved states instead of steady states
clear;clc;
% Parameters
dimS = 2;
omegaS = 10;
...
for j = 1:length(sigma)
    ...
    save('noisyBath_wS10_wB10_TB10_gjj1_gamma0.01_dtInt10_Ntrials10000.mat');
end
clear;clc;
% Parameters
dimS = 2;
...
for j = 1:length(sigma)
    ...
    c = collisionModelSpinBath(dimS,omegaS,dimB,omegaB,TB,gamma,dtInt,gsb,rho0);
    [rhoSt{j}, Jzt(j,:), Wt(j,:), St{j}, Et{j}, NjumpsAv(j), t(j,:)] = c.runTheWorkersNoise(Ntrials,tstep(j),numStep,sigma(j));
    save('noisyBath_wS10_wB10_TB10_gxx1_gamma0.01_dtInt10_Ntrials10000.mat');
end
