...
classdef collisionModel < handle
    ...
    properties
        ...
        ESBmESB
        pthB
        rhoB %thermal Boltzmann weights of bath
    ...
    methods
        function obj = collisionModel(ds,Nb,db,ob,Tb,gb,dtb)
            obj.dS = ds;
            obj.NB = Nb;
            obj.HS = zeros(ds);
...
%                 obj.HB{n} = diag( (-JB:JB)*obj.omegaB(n) );
                ...
                obj.SB(n) = obj.getEntropy(obj.rhoB{n});
                obj.UB(n) = sum(sum(obj.rhoB{n}.*obj.HB{n}));
            end
		...
		%NOTE: Using this as Lindblad dissipator in ME assumes Poisson point process, i.e. short interaction times!
            ...
            for j = 1:obj.dB(n)
                for l = 1:obj.dB(n)
                    ...
                    K(j,l) = 1;                     
                    [r,c] = find(kron(K,ones(obj.dS)));
                    tmp = obj.S{n}(unique(r),unique(c))*sqrt(obj.rhoB{n}(j,l));
        ...
        function getSteadyVal(obj)
            for n = 1:obj.NB
                rhoIn = kron(obj.rhoB{n},obj.rhoSS{1});
        ...
        % observable!)
            ...
            for n=1:obj.NB
                ...
                W(n,1) = trace( kron(obj.rhoB{n},U0{n}.*obj.rhoS0)*WOps{n} );
            end
            ...
            for j = 2:Nt
                ...
                for n=1:obj.NB
                    W(n,j) = trace( kron(obj.rhoB{n},U0{n}.*rhot)*WOps{n} );
                end
                ...
                if nt < nJump %no jump occurs in this step
                    ...
                    while interactionLoop %necessary, bath interactions may extend over 1 step
                        ...
                        if tnow < tJump %jump hasn't happened yet
                            ...
                            rhotmp = (V')* (kron(obj.rhoB{bath},rhoSnow) * V);
                ...
                if nt < nJump %no jump occurs in this step
                    ...
                    while interactionLoop %necessary, bath interactions may extend over 1 step
                        ...
                        if tnow < tJump %jump hasn't happened yet
                            ...
                            S(2,nt) = S(2,nt)-obj.getEntropy(obj.rhoB{bath});
                            ...
                            rhotmp = kron(obj.rhoB{bath},rhoSnow);       
