...
theta(1,1) = 0;
theta(theta==max(max(theta))) = 0.25*pi;
division = 0:0.025:0.25;
contourf(nbar,gammat,theta/pi,division);
hold on;
plot(nbar,0.8./nbar,'r:','linewidth',3);
axis([0 10 0 1]);
...
F1 = -FI./F0;
F1(F1>1.65) = 1.651;
division = 1:0.05:1.65;
F1(F1==max(max(F1))) = 1.651;
