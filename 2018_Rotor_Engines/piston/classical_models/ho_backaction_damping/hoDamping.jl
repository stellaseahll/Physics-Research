...
function runPendulumEngine2ModeDrive!(avgs, snaps, Ntrials, tmax, Nt,
  ...
  @inbounds @simd for i=1:Ntrials
    ...
    avgs[1,7] += ait[i]
    avgs[1,8] += nati
    avgs[1,9] += g*nati*sinXti*pt[i]/I
    avgs[1,10] += kappa*g*cosXti*fH2ti*(nH-nati)
    avgs[1,11] += kappa*g*cosXti*fC2ti*(nC-nati)
    avgs[1,12] += kappa*fH2ti*(nH-nati)
    avgs[1,13] += kappa*fC2ti*(nC-nati)
  end
  avgs[1,1] = 0.0 #TIME
  ...
  for nt = 1:Nt
    ...
    if iSave==nSave
      ...
      @inbounds @simd for i=1:Ntrials
        ...
        fH2ti = fHti*fHti
        fC2ti = fCti*fCti
        nXti = (fH2ti*nH + fC2ti*nC)/(fH2ti+fC2ti)
        kXti = kappa*(fH2ti + fC2ti)/2
        nati = ait[i]*ait[i] + art[i]*art[i]
        dart[i] = -kXti*art[i]*dt + g*cosXti*ait[i]*dt +
                  - fHti*skHdW*w1 -fCti*skCdW*w3
        dait[i] = -kXti*ait[i]*dt - g*cosXti*art[i]*dt +
                  - fHti*skHdW*w2 -fCti*skCdW*w4
        xt[i] += pt[i]/I*dt
        pt[i] += g*nati*sinXti*dt - gamma*pt[i]*dt + dampdW*randn() +cosXti*skHdW*(ait[i]*w1 - art[i]*w2)-cosXti*skCdW*(ait[i]*w3 - art[i]*w4)
        art[i] += dart[i]
        ait[i] += dait[i]
        #save averages
        nati = ait[i]*ait[i] + art[i]*art[i]
        ...
        avgs[jSave,7] += ait[i]
        avgs[jSave,8] += nati
        avgs[jSave,9] += g*nati*sinXti*pt[i]/I
        avgs[jSave,10] += kappa*g*cosXti*fH2ti*(nH-nati)
        avgs[jSave,11] += kappa*g*cosXti*fC2ti*(nC-nati)
        avgs[jSave,12] += kappa*fH2ti*(nH-nati)
        avgs[jSave,13] += kappa*fC2ti*(nC-nati)
      end
      avgs[jSave,1] = nt*dt
      jSave += 1
    else
