...
%
% Computing the FI is the costly step
%  My 16GB RAM Mac could handle up to 9 probes with its memory, using the
%  standard mldivide method for the operator inversion
%  Can improve mem efficiency using iterative scheme pcg(). But need to go
%  to high iterations to converge. But this still pays off and may be
%  faster... See and uncomment below in catchFish function
classdef spSinglePassXY < handle
    ...
    properties
        ...
        rhoPsingle %initial 1-probe state
    ...
    methods
        function obj = spSinglePassXY(g,gammat,nbar,dnbar,rhoP0,np,isSteadOp)
        ...
        function getOperators(obj)
            ...
            obj.JSmat{1} = kron(obj.JxS,speye(obj.dP^obj.NP));
            obj.JSmat{2} = kron(obj.JyS,speye(obj.dP^obj.NP));
            obj.JSmat{3} = kron(obj.JzS,speye(obj.dP^obj.NP));
            obj.JSmat{4} = kron(obj.JpS,speye(obj.dP^obj.NP));
            obj.JSmat{5} = kron(obj.JmS,speye(obj.dP^obj.NP));
            obj.JSmat{6} = kron(obj.JmS*obj.JpS,speye(obj.dP^obj.NP)); % |g><g|
            obj.JSmat{7} = kron(obj.JpS*obj.JmS,speye(obj.dP^obj.NP)); % |e><e|
            ...
            for j = 1:obj.NP
                ...
                obj.Umat{j} = -1i*sin(obj.G)*(egge + egge.'); % XX+YY swap, U = exp(-1i*G*(|eg><ge| + h.c.))
                obj.Umat{j} = obj.Umat{j} + speye(obj.dS*obj.dP^obj.NP); 
                obj.Umat{j} = obj.Umat{j} + (cos(obj.G)-1)*(gege+egeg);
            end
        end
        
        function prepareProbeState(obj,rho) %make sparse
            if (length(rho)== obj.dP)
                obj.rhoPsingle = sparse(rho);
% NOT ALLOWED for independent single probes!                
%             elseif (length(rho)== obj.dP^obj.NP)
%                 obj.rhoP0 = sparse(rho);
            else
                error('Check input state of probe');
            end
            obj.rhoP0=1;
            for i = 1:obj.NP
                obj.rhoP0 = kron(obj.rhoPsingle,obj.rhoP0);
            end
        end
        ...
        function getSteadyStates(obj) %compute all reduced system steady states for the nbar's
            %This assumes identical qubit ancilla states obj.rhoPsingle
            pA = obj.rhoPsingle(1,1);
            cA = obj.rhoPsingle(2,1);
            for i=1:3
        ...
        function F = catchFish(obj,rho,drho)
            ...
            elseif obj.alg==2 %use iterative pcg algorithm for F
                ...
                F = full( 2*drhot(:).' * ( pcg(A, drho(:),obj.tol,obj.maxit) ) );
