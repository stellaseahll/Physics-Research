...
function runPendulumEngine2ModeDrive!(avgs, snaps, Ntrials, tmax, Nt,
  ...
  @inbounds @simd for i=1:Ntrials
    ...
    fHti = 0
    fCti = 0
    if (angle>=-0.375*2*pi/2.0+0.5*pi && angle<=0.375*2*pi/2.0+0.5*pi)
      fHti = 1
    elseif (angle>=-0.375*2*pi/2.0+1.5*pi && xt[i]<=0.375*2*pi/2.0+1.5*pi)
      fCti = 1
    end
    fH2ti = fHti*fHti
    fC2ti = fCti*fCti
  ...
  for nt = 1:Nt
    ...
    if iSave==nSave
      ...
      @inbounds @simd for i=1:Ntrials
        ...
        if (angle>=-0.375*2*pi/2.0+0.5*pi && angle<=0.375*2*pi/2.0+0.5*pi)
          fHti = 1
        elseif (angle>=-0.375*2*pi/2.0+1.5*pi && angle<=0.375*2*pi/2.0+1.5*pi)
          fCti = 1
        end
        fH2ti = fHti*fHti
        fC2ti = fCti*fCti
        ...
        rflip = rand()
        if (nati[i]==0 && rflip<pH1*fH2ti+pC1*fC2ti)
          nati[i] = 1
          #r = rand()
          #if (r<0.25)
          #  pt[i] += 1
          ...
          #  pt[i] -= 1
          #end
        elseif (nati[i]==1 && rflip<pH2*fH2ti+pC2*fC2ti)
          nati[i] = 0
          #r = rand()
          #if (r<0.25)
          #  pt[i] += 1
