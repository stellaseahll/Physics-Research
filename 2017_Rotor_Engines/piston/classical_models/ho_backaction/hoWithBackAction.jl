    ...
    if iSave==nSave
      ...
      @inbounds @simd for i=1:Ntrials
        ...
        dart[i] = -kXti*art[i]*dt + g*cosXti*ait[i]*dt +
                  - fHti*skHdW*r1 -fCti*skCdW*r3
        dait[i] = -kXti*ait[i]*dt - g*cosXti*art[i]*dt +
                  - fHti*skHdW*r2 -fCti*skCdW*r4
        dpt = g*nati*sinXti*dt+cosXti*(skHdW*(ait[i]*r1 - art[i]*r2)-skCdW*(ait[i]*r3 - art[i]*r4))
        xt[i] += pt[i]/I*dt
        pt[i] += dpt
        art[i] += dart[i]
        ait[i] += dait[i]
