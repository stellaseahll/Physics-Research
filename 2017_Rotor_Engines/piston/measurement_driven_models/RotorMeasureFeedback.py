...
parser.add_argument("-k", "--k", dest="k_value", type=float, metavar='<float>', default=1.0, help="K value (default=1.0)")
parser.add_argument("-kf", "--kf", dest="kf_value", type=float, metavar='<float>', default=0.01, help="K value (default=1.0)")
parser.add_argument("-km", "--km", dest="km_value", type=float, metavar='<float>', default=0.01, help="K value (default=1.0)")
...
# Measurement and Feedback Rates
kf=args.kf_value #feedback rate
km=args.km_value #measurement rate
...
H_q= tensor(sigmaz,qeye(d2))
omega = 10
# Interaction Hamiltonian
Hint=g*tensor(sigmap*sigmam,cos_Phi)

# Total Hamiltonian
H=H_r+omega*H_q+Hint
...
A = sqrt(2*km) * tensor(qeye(2),fHot_Phi)
B = sqrt(kf*kf/8/km)*tensor(sigmax,qeye(d2))
H = H + 0.5*(A*B + B*A)
...
solution=mesolve(H, rho, timesteps, [C_mf,C_cold,C_cold_d], [P,ELz,ELz2,H_q],options=options)
...
ELz = solution.expect[1]
ELz2 = solution.expect[2]
EN = solution.expect[3]
Hke = lz*lz/2/MI
p1 = np.zeros(NT)
p2 = np.zeros(NT)
# for j in range(0,NT):
...
dataThermo=np.vstack((timesteps,W1,W2,ELz,ELz2,EN,p1,p2))
#     print(j)
#     x = solution.states[j]
#     y = x.ptrace(1)
...
file_data_store('k1_nh1nc0_datashift%s.dat'%(run),dataThermo,numtype="real")
file_data_store('k1_nh1nc0_stateshift%s.dat'%(run),solution.states[NT-1],numtype="complex")
file_data_store('k1_nh1nc0_probR%s.dat'%(run),probR,numtype="real")
file_data_store('k1_nh1nc0_eigR%s.dat'%(run),eigR,numtype="real")
