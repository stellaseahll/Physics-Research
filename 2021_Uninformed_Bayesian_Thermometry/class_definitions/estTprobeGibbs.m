...
classdef estTprobeGibbs < handle
    ...
    methods
        ...
        function obj = estTprobeGibbs(Tvals,N_P,d,whichPrior)
            
            if nargin < 4
                whichPrior = 'Jeffreys'; %by default assume Jeffreys
            end
            obj.d = d;
            obj.T = Tvals(:); %make column vector
            obj.NT = length(Tvals);
            obj.NP = N_P;
        ...
        function allPost_Jsqrt(obj)
            %Assumes Jeffreys prior using parametrization in x=sqrt(p) with
            %p the qubit excitation probability. (No divergence for positive T)
            %sets posterior handles and computes norms for all possible
            %outcomes n = 0..NP
            %uses array-valued integration routine
            %n = (0:obj.NP);
            tic
            obj.priorH = @(x) 4/pi./sqrt(1-x.^2);
            obj.normPost = zeros(1,obj.NP+1);
            obj.postH = @(x) obj.priorH(x).*binopdf( (0:obj.NP), obj.NP, x.^2 );
            obj.normPost = integral(obj.postH,0,1/sqrt(2),'ArrayValued',true);
            toc
