...
f = integral(@(x) myHermite(m,x).*myHermite(n,x).*exp(-x.^2).*exp(-(x+a).^2/2/s^2),-inf,inf)/sqrt(2^(m+n)*factorial(m)*factorial(n)*pi); 
% f = integral(@(x) myHermite(m,x).*myHermite(n,x).*g(x).*exp(-(x+a).^2/2/s^2),-inf,inf)/sqrt(2^(m+n)*factorial(m)*factorial(n)*pi); 
%
% kend = min([m n]);
% % f = 0;
