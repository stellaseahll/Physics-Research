...
%     plot(real(diag(sprhog))); hold on; plot(real(diag(sprhoe)));
    ...
    %% Get heat flows
%     W(i) = 1i*G*wd*trace(kron(sm-sp,idHO)*sprhoSS);
    W2(i) = 1i*G*wd*trace(kron(sm-sp,projM)*sprhoSS);
    Qh2(i) = trace((kh*(nh+1)*diss(sprhoSS,kron(sm,idHO)) +kh*nh*diss(sprhoSS,kron(sp,idHO)))*(H + Hrot));
    Qc2(i) = trace((kc*(nc+1)*diss(sprhoSS,A) +kc*nc*diss(sprhoSS,Adg))*(H + Hrot));
...
% filename = sprintf('DiffG_Projector_nc%.3f.mat',nc);
% save(filename,'nh','nc','kh','kc','delta','g','G','W','Qh','Qc','ws','wp');
