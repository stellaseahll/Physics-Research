...
probxg = [probxg full(real(diag(sprhog)))];
probxe = [probxe full(real(diag(sprhoe)))];
% 
% %% Get heat flows
...
Ws(i) = trace((km*diss(sprhoSS,kron(idQ,projP))+km*diss(sprhoSS,kron(sx,projM)))*Hq);
...
Wp(i) = trace((km*diss(sprhoSS,kron(idQ,projP))+km*diss(sprhoSS,kron(sx,projM)))*Hp);
...
Wint(i) = trace((km*diss(sprhoSS,kron(idQ,projP))+km*diss(sprhoSS,kron(sx,projM)))*Hint);
Qhtot(i) = Qhs(i)+Qhp(i)+Qhint(i);
Qctot(i) = Qcs(i)+Qcp(i)+Qcint(i);
Wtot(i) = Ws(i)+Wp(i)+Wint(i);
end
X = [0.7:0.1:Xend Xend];
clear sprhog sprhoe sprhoSSAll spLtot spLm1 spLm2 spLcg spLce spLh1 spLh2 spUg spUe;
filename = sprintf('km%.5f_kh%.5f_kc%.5f.mat',km,kh,kc);
save(filename);
