...
load('ThesisCoherentData_Global_Step_wp1_ws100_g2.50_nh1.00_nc0.10.mat');
work = -real(W)/ws/kh;
eff = real(-W)./real(Qh);
...
map = [1 1 1; red];
k = [0; 1];
map = interp1(k,map,linspace(0,1,12));
% map(1,:) = [];

figD = figure;
% [a b] = contour(log10(G),log10(kc),work,[-0.1 0]);
% x =a(1,2:end);
% y =a(2,2:end);
...
% hold on;
contourf(log10(G),log10(kc),work,0:0.05:0.6,'linecolor','none');
colormap(map);
ylabel('$\kappa_{\rm c}/\omega$','interpreter','latex','FontSize',12);
% xlabel('$\xi/\omega$','interpreter','latex','FontSize',12);
...
% h.Label.string = 0
h.Units = 'centimeters';
set(h,'Ticks',[0.001 0.2 0.4 0.6],'TickLabels',{0 0.2 0.4 0.6});
...
print('coherentWorkContour', '-dpdf', '-r600')
close(figD);
figD = figure();
...
% fill(negwork(1,:),negwork(2,:),blue,'edgecolor','none');
% hold on;
contourf(log10(G),log10(kc),eff,min(min(eff)):0.1:0.95,'linecolor','none');
colormap(map);
ylabel('$\kappa_{\rm c}/\omega$','interpreter','latex','FontSize',12);
xlabel('$\xi/\omega$','interpreter','latex','FontSize',12);
...
print('coherentEffContour', '-dpdf', '-r600')
close(figD);