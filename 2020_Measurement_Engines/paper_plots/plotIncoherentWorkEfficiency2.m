...
xlabel('$x_0/x_{\mathrm th}$','interpreter','latex','FontSize',paper.labelSize);
...
set(ax,'XTick',[0.5:0.5:2.5]);
set(ax,'YTick',0:0.2:1.0);
text(0.75,0.85,'(a)','interpreter','latex','FontSize',paper.labelSize);
...
text(0.75,0.85,'(b)','interpreter','latex','FontSize',paper.labelSize);
set(ax,'FontSize',paper.fontSize);

ax=subplot(1,3,3); 
a = 0.9;
%fill([0 0 XEnd XEnd 0], [-0.1 1 1 -0.1 -0.1],[a a a],'linestyle','none');
%semilogy(X,qc1,'Color',c(1,:),'LineWidth',1);
%hold on
semilogy([0 max(X)],[1 1],'k--','Linewidth',1);
hold on
semilogy(X,qc1,'Color',c(1,:),'LineWidth',1);
semilogy(X,qc2,'Color',c(2,:),'LineWidth',1);
semilogy(X,qc3,'Color',c(4,:),'LineWidth',1);
semilogy(X,qc4,'Color',c(5,:),'LineWidth',1);
xlabel('$x_0/x_{\mathrm th}$','interpreter','latex','FontSize',paper.labelSize);
ylabel('$\dot{Q}_c/\kappa_m k_B T_c {\rm ln} 2 $','interpreter','latex','FontSize',paper.labelSize);
%xlim([0 max(X)]);
xlim([0.5 max(X)]);
%ylim([-0.02 0.9]);
ylim([0.1 1000]);
...
text(0.75,250,'(c)','interpreter','latex','FontSize',paper.labelSize);
ff=fill([0 0 XEnd XEnd 0], [0.1 1000 1000 0.1 0.1],[a a a],'linestyle','none');
set(ax,'FontSize',paper.fontSize);
...
print('figWorkEffNEW.pdf', '-dpdf', '-r600')
